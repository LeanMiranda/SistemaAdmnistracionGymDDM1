...
const ListaRutina = () => {
  ...
  const getUsersDB = async () => {
    ...
    await db.transactionAsync(async (tx) => {
      result = await databaseConection.getAllRutina(tx);
    }, readOnly);
    // seteara test
    return result;
  };
  ...
  const listItemView = (item) => {
    return (
      <View key={item.usuario} style={styles.listItemView}>
        ...
        <MyText text="dia rutina" style={styles.text} />
        <MyText text={item.diarutina} style={styles.text_data} />

        <MyText text="Ejercicio" style={styles.text} />
        <MyText text={item.ejercicio} style={styles.text_data} />
